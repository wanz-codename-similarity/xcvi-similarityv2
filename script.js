...
// ===== Attack Modal logic (override old option toggle) =====
(function initAttackModal() {
  ...
  if (!openBtn || !modal || !optionsContainer) return;

  // disable earlier toggle behavior if exists
  mainOptionBtn.onclick = null;

  // helper
  function showAttackModal() {
    modal.classList.add('show');
    modal.setAttribute('aria-hidden', 'false');
    // center first selected or first item
  ...
  // close handlers
  modalClose.addEventListener('click', hideAttackModal);
  cancelBtn.addEventListener('click', hideAttackModal);
  // click outside box closes modal
  modal.addEventListener('click', (ev) => {
    if (ev.target === modal) hideAttackModal();
  });

  // item click: pilih (klik TENGAH area is same as click)
  ...
  function setSelectedItem(el) {
    ...
    // update global selected vars
    selectedEndpoint = el.dataset.endpoint || selectedEndpoint;
    selectedLabel = el.textContent.trim() || selectedLabel;
    // update button label immediately to reflect choice
    mainOptionBtn.textContent = selectedLabel;
    // show confirm button briefly (UX)
    if (confirmBtn) {
  ...
  // click listener on container (delegation)
  optionsContainer.addEventListener('click', (ev) => {
    const item = ev.target.closest('.attack-item');
    if (!item) return;
    // center item and mark selected
    item.scrollIntoView({behavior:'smooth', inline:'center'});
    setSelectedItem(item);
    // short delay then close modal to satisfy "klik aja tengah untuk memilih"
    setTimeout(() => {
      // you may want auto-close; keep it fast but visible
      hideAttackModal();
      showToast(`Dipilih: ${selectedLabel}`);
    }, 350);
  });

  // keyboard accessibility (left/right to navigate, Enter to choose)
  ...
  // make items focusable & set initial selected
  optionsContainer.querySelectorAll('.attack-item').forEach(it => {
    it.tabIndex = 0;
  });
  // default selection: first item if none selected
  (function ensureDefault() {
    const cur = optionsContainer.querySelector('.attack-item.selected');
    if (!cur) {
  ...
  // optional: confirm button click (if you want explicit confirm)
  if (confirmBtn) {
  ...
  // touch drag smoothing is handled by browser via overflow-x:auto
})();

// ===== Attack Modal Control =====
const openAttackModalBtn = document.getElementById('openAttackModalBtn');
const attackModal = document.getElementById('attackModal');
const attackModalClose = document.getElementById('attackModalClose');
const attackItems = document.querySelectorAll('.attack-item');
let selectedAttack = null;

// buka modal
openAttackModalBtn.addEventListener('click', () => {
  attackModal.classList.add('show');
});

// tutup modal
attackModalClose.addEventListener('click', () => {
...
// klik salah satu item attack
attackItems.forEach((item) => {
  item.addEventListener('click', () => {
    attackItems.forEach((i) => i.classList.remove('selected'));
    item.classList.add('selected');
    selectedAttack = item.dataset.endpoint;

    // update tombol utama biar kelihatan
    document.getElementById('mainOptionBtn').innerHTML =
      `🎯 ${item.textContent}`;
    attackModal.classList.remove('show');
  });
});
...
function log(msg){const el=document.createElement('div');el.textContent='> '+msg;document.getElementById('log').prepend(el);}
...
async function sendBtnAction() {
  ...
  const endpoint = selectedEndpoint || 'send';
  const label = selectedLabel || endpoint;

  // validasi cepat
  if (!to) {
    showToast('Isi nomor target dulu bro!');
    showPopup('Nomor Target Kosong', 'Masukkan nomor target (contoh: 628xx) sebelum mengirim.', { type: 'warn' });
    return;
  }

  if (!s.name || !s.phone) {
    showToast('Belum ada session aktif. Pair dulu!');
    showPopup('Session Kosong', 'Generate pairing code dulu untuk membuat session sebelum mengirim.', { type: 'warn' });
    return;
  }

  // optional: validasi format nomor tujuan (hanya digit, minimal 8 digit)
  const onlyDigits = to.replace(/\D/g, '');
  if (onlyDigits.length < 8) {
    showToast('Nomor target tampak tidak valid!');
    showPopup('Nomor Tidak Valid', 'Periksa kembali nomor target (pastikan diawali 62 dan minimal 8 digit).', { type: 'warn' });
    return;
  }
  ...
  btn.innerHTML = `<svg class="spinner" viewBox="0 0 50 50" style="width:18px;height:18px">
    <circle cx="25" cy="25" r="20" stroke="rgba(255,255,255,0.4)" stroke-width="4" fill="none"></circle>
  </svg>`;

  showToast(`Process → ${label}...`);
  log(`Sending ${label} to ${to} via session ${s.name}_${s.phone}`);
  ...
  try {
    ...
    try {
      ...
      showToast('Server balas bukan JSON ❌');
      log('Raw response (not JSON): ' + String(raw).slice(0, 400));
      throw new Error('Invalid JSON response from server');
    }

    if (data && data.ok) {
      showToast(`Success → ${label}`);
      log(`Terkirim: ${label} -> ${to} (session: ${s.name}_${s.phone})`);
    } else {
      const errMsg = (data && (data.error || data.message)) || 'Unknown error';
      showToast(`❌ Gagal kirim (${errMsg})`);
      log(`Send error: ${errMsg}`);
      // kalau backend kembalikan detail kritis, tampilkan popup
      if (res.status >= 500) {
        showPopup('Server Error', `Backend error (${res.status}). Cek server atau lihat console.`, { type: 'error' });
      }
    }
    ...
    showToast('❌ Server tidak merespon atau error');
    log('Fetch/send error: ' + em);
    showPopup('Error Koneksi', 'Gagal terhubung atau menerima respons dari server. Cek koneksi atau server.', { type: 'error' });
  } finally {
    // restore UI
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!openAttackModalBtn || !attackModal) {
    console.error("❌ Elemen modal attack tidak ditemukan di DOM");
    return;
  }

  // buka modal
  openAttackModalBtn.onclick = () => {
    attackModal.classList.add('show');
    console.log("✅ Modal attack dibuka");
  };

  // tutup modal (tombol X)
  if (attackModalClose) {
    attackModalClose.onclick = () => {
      attackModal.classList.remove('show');
      console.log("✅ Modal attack ditutup (tombol X)");
    };
  }
  ...
  attackModal.addEventListener('click', (e) => {
    if (e.target === attackModal) {
      attackModal.classList.remove('show');
      console.log("✅ Modal attack ditutup (klik luar)");
    }
  });
  ...
  if (cancelBtn) {
    cancelBtn.onclick = () => {
      attackModal.classList.remove('show');
      console.log("✅ Modal attack ditutup (batal)");
    };
  }

  // klik salah satu pilihan attack
  attackItems.forEach(item => {
    item.addEventListener('click', () => {
      attackItems.forEach(i => i.classList.remove('selected'));
      item.classList.add('selected');

      // update variabel global (buat send)
      window.selectedEndpoint = item.dataset.endpoint;
      window.selectedLabel = item.textContent.trim();

      // ubah label tombol utama
      if (mainOptionBtn) {
        mainOptionBtn.innerHTML = `🎯 ${window.selectedLabel}`;
      }

      // tutup modal
      attackModal.classList.remove('show');
      console.log("dipilih:", window.selectedLabel);
    });
  });
});
