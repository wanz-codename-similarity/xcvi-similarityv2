...
async function sendBtnAction() {
  ...
  // popup cmatrix elements (may not exist)
  ...
  try {
    ...
    // Use explicit query param key to avoid malformed URL
    const url = `/api/connect?${encodeURIComponent(endpoint)}`;
    ...
    try {
      ...
      safeLog('Raw response: ' + String(raw).slice(0, 400));
      throw new Error('Invalid JSON');
    }

    // beri waktu biar animasi kebaca penuh
    ...
    if (data && data.ok) {
      ...
      if (sendModal) {
        try {
          if (sendInfo) sendInfo.innerHTML = `<div style="color:#f55;text-align:center;">${errMsg}</div>`;
          setTimeout(() => {
            if (sendModal) {
              sendModal.classList.remove('show');
              sendModal.setAttribute('aria-hidden', 'true');
      ...
      if (res.status >= 500) {
        safeShowPopup('Server Error', `Backend error (${res.status}). Cek server.`, { type: 'error' });
      }
    }
  } catch (err) {
    ...
    if (sendModal) {
      try {
        if (sendInfo) sendInfo.innerHTML = `<div style="color:#f55;text-align:center;">${msg}</div>`;
        setTimeout(() => {
          if (sendModal) {
            sendModal.classList.remove('show');
            sendModal.setAttribute('aria-hidden', 'true');
    ...
    // cleanup interval & modal if still showing
    try {
      if (intv) { clearInterval(intv); intv = null; }
      if (sendMatrix) sendMatrix.style.display = 'none';
      if (sendInfo) sendInfo.style.display = 'none';
