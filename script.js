...
let selectedLabel = "Pilih Menu Attack";
...
document.querySelectorAll(".optionBtn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    mainOptionBtn.textContent = selectedLabel;
    optionsBlock.classList.remove("show");
...
if (appPanel && loginBanner) {
  const observer = new MutationObserver(() => {
    if (appPanel.style.display === "none" || appPanel.hidden) {
      loginBanner.style.display = "block";
    } else {
      loginBanner.style.display = "none";
    }
  });
  observer.observe(appPanel, { attributes: true, attributeFilter: ["style"] });
}
