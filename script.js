...
async function sendBtnAction() {
  ...
  const endpoint = btn?.dataset?.endpoint || selectedEndpoint || 'send';
  const label = btn?.dataset?.label || selectedLabel || endpoint;
  ...
  // validasi format nomor (hanya digit, minimal 8)
  ...
  btn.querySelector('.send-text').textContent = `Processing ${label}...`;

  showToast(`Process → ${label}...`);
  log(`Sending ${label} to ${to} via session ${s.name}_${s.phone}`);

  // popup cmatrix elements
  ...
  if (sendModal && matrixPre) {
    ...
    sendMatrix.style.display = 'block';
    sendModal.classList.add('show');
    sendModal.setAttribute('aria-hidden', 'false');

    sendTarget.textContent = onlyDigits;
    sendOption.textContent = label;

    // animasi cmatrix
    const chars = '01█▒▓░<>[]()#@*+—\\/|';
    const lines = 12;
    const buffer = new Array(lines).fill('').map(randomLine);
    ...
    const intv = setInterval(() => {
      ...
      if (Math.random() < 0.12) {
        ...
        for (let i = 0; i < text.length; i++) arr[pos + i] = text[i];
        buffer[mid] = arr.join('');
      }
    }, 80);

    // stop matrix & tunjukkan info
    setTimeout(() => {
      clearInterval(intv);
      sendMatrix.style.display = 'none';
  ...
  try {
    ...
    try {
      ...
      log('Raw response: ' + String(raw).slice(0, 400));
      throw new Error('Invalid JSON');
    }

    await new Promise(r => setTimeout(r, 3000)); // biar animasi kebaca penuh
    ...
    if (data && data.ok) {
      ...
      if (sendModal) {
        sendInfo.innerHTML = `<div style="color:#f55;text-align:center;">${errMsg}</div>`;
        setTimeout(() => {
          sendModal.classList.remove('show');
          sendModal.setAttribute('aria-hidden', 'true');
        }, 2000);
      ...
      if (res.status >= 500) {
        showPopup('Server Error', `Backend error (${res.status}). Cek server.`, { type: 'error' });
      }
    }
  } catch (err) {
    const msg = err?.message || String(err);
    showToast('❌ Server tidak merespon');
    log('Fetch/send error: ' + msg);
    showPopup('Error Koneksi', 'Gagal terhubung ke server. Cek koneksi atau backend.', { type: 'error' });
    ...
    btn.querySelector('.send-text').textContent = 'Send Bug';
  }
}
