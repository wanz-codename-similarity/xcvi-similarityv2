...
/* ===== Slider pilihan attack (carousel) ===== */
...
function clearSelectedSlides() {
  document.querySelectorAll('#optionsSlider .slide').forEach(s => s.classList.remove('selected'));
}
...
// klik slide -> pilih
optionsSlider.querySelectorAll('.slide').forEach(sl => {
  ...
  sl.addEventListener('keydown', (ev) => {
    if (ev.key === 'Enter' || ev.key === ' ') {
      ev.preventDefault(); selectSlide(sl);
    }
  });
});

// default: pilih slide pertama jika belum ada session selection
(function ensureDefaultSelected() {
  const cur = optionsSlider.querySelector('.slide.selected');
  if (!cur) {
    const first = optionsSlider.querySelector('.slide');
    if (first) selectSlide(first);
  }
})();
...
optPrev.addEventListener('click', () => {
  ...
  const w = optionsSlider.clientWidth;
  optionsSlider.scrollBy({ left: -w * 0.6, behavior: 'smooth' });
});
...
// touch drag support (better UX on mobile)
let isDown = false, startX, scrollLeft;
optionsSlider.addEventListener('mousedown', (e) => {
  isDown = true; optionsSlider.classList.add('active-drag');
  startX = e.pageX - optionsSlider.offsetLeft;
  scrollLeft = optionsSlider.scrollLeft;
});
optionsSlider.addEventListener('mouseleave', () => { isDown = false; optionsSlider.classList.remove('active-drag'); });
optionsSlider.addEventListener('mouseup', () => { isDown = false; optionsSlider.classList.remove('active-drag'); });
optionsSlider.addEventListener('mousemove', (e) => {
  if (!isDown) return;
  e.preventDefault();
  const x = e.pageX - optionsSlider.offsetLeft;
  const walk = (x - startX) * 1; // scroll-fast factor
  optionsSlider.scrollLeft = scrollLeft - walk;
});

// touch events
optionsSlider.addEventListener('touchstart', (e) => {
  startX = e.touches[0].pageX - optionsSlider.offsetLeft;
  scrollLeft = optionsSlider.scrollLeft;
});
optionsSlider.addEventListener('touchmove', (e) => {
  const x = e.touches[0].pageX - optionsSlider.offsetLeft;
  const walk = (x - startX);
  optionsSlider.scrollLeft = scrollLeft - walk;
});

// auto-center selected slide when window resizes
window.addEventListener('resize', () => {
  const sel = optionsSlider.querySelector('.slide.selected');
  if (sel) sel.scrollIntoView({ behavior: 'smooth', inline: 'center' });
});

// keep mainOptionBtn behavior compatible
// (mainOptionBtn previously toggled class show/active; keep it)
mainOptionBtn.addEventListener('click', () => {
  // toggle visual only, since options are now a slider
  const isActive = mainOptionBtn.classList.toggle('active');
  // when active, focus slider for keyboard nav
  if (isActive) {
    optionsSlider.focus();
    // show toast hint
    showToast('Geser pilihan → / klik panah');
  } else {
