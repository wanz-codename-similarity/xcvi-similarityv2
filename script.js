...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sendBtn) sendBtn.onclick = () => sendBtnAction();
});
...
if (toggleBtn) {
  ...
  (function () {
    ...
    // Ensure muted (biar pasti autoplay jalan di mobile)
    v.muted = true;
    v.playsInline = true;
    v.loop = true;

    // Try to play; jika browser menolak, tangani error dan coba lagi saat interaksi
    function tryPlay() {
      v.play().catch((err) => {
        // autoplay may be blocked; listen for a user interaction and try again
        // kita pasang hanya sekali listener untuk tidak spam
    ...
    // Jika tab visibility berubah (mis. user balik ke tab), coba mainkan lagi
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') {
        tryPlay();
      } else {
        // beberapa browser pause saat tidak terlihat — tetap upaya play balik saat visible
        // tidak memaksa play saat hidden karena browser bisa men-suspend resource
      }
    });

    // Jika video ended (seharusnya tidak karena loop=true), pastikan restart
