...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ===== FEATURE CONTROL (toggle per session) =====
if (method === "POST" && path.includes("feature")) {
  const body = await parseBody(req);
  const { name, phone, feature, value } = body || {};

  try {
    const resp = await axios.post(`${base}/feature`, { name, phone, feature, value }, axiosOpt);
    return res.status(200).json(resp.data);
  } catch (e) {
    return res.status(500).json({ ok:false, error:"Gagal update fitur", details:e.message });
  }
}
